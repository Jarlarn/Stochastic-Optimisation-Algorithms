...
def plot_simulation_results(history: Dict[str, List[float]], title: str = "Truck Simulation Results"):
...
def test_controller(chromosome: List[float], 
    ...
    if save_plot:
        plt.savefig(f"simulation_results_slope{slope_index}_dataset{data_set_index}.png")
    else:
        plt.show()
...
def main():
    ...
    print(f"Loaded chromosome with ni={ni}, nh={nh}, no={no}")
    print(f"Best fitness from optimization: {data.get('fitness', 'N/A')}")
    
    # Test on all slopes and datasets
    for slope_index in [0, 1]:
        for data_set_index in [0]:
            test_controller(
