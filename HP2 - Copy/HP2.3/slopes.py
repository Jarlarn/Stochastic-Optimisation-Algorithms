...
def get_slope_angle(x: float, slope_index: int, data_set_index: int) -> float:
    ...
    All slopes are inherently bounded: 0 <= alpha <= ALPHA_MAX for all x.
    Each slope has its own min/max bounds within [0, ALPHA_MAX].
    """
    ...
    if data_set_index == 1:
        if slope_index == 1:
            ...
            # Sustained high angle, similar to test set
            alpha_deg = (
            ...
            # Long steep section, negative cosine for sustained descent
            alpha_deg = (
            ...
            # Nearly constant high angle with small oscillation
